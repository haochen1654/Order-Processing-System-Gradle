...
public class Client {
  ...
  public Problem newProblem(String name, long seed) throws IOException {
    ...
    URL url = new URL(endpoint + "/interview/challenge/new?auth=" + auth +
                      "&name=" + name + "&seed=" + seed);
  ...
  /**
   * solveProblem submits a sequence of actions and parameters as a solution to
   * a test problem. Returns test result.
   */
  public String solveProblem(String testId, Duration rate, Duration min,
                             Duration max, List<Action> actions)
      throws IOException {
    ...
    HttpResponse response =
        client.post(url, new UnencodedStringMessage(
                             solution.encode(),
